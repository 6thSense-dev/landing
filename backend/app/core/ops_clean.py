...
def validate_manifest(doc):
    ...
    if doc.get('schema') != SCHEMA or not matches(r'[a-zA-Z0-9_-]{1,120}', doc.get('run_id', '')):
    ...
    for output in outputs:
        key = output.get('key', '')
        if not isinstance(key, str) or not key.startswith(f"clean/{doc['run_id']}/") or any(p in ('', '.', '..') for p in key.split('/')):
            raise ValueError('Output outside this clean run')
        if (not key.endswith(('.mp4', '.m3u8')) or not pinned_version(output.get('version_id'))
