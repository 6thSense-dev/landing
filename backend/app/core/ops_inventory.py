...
def build_coverage(objects, *, bucket, listing_complete, stop_reason=None):
    prefixes = {}
    ...
    for obj in objects:
        ...
        if key.endswith('/'):
            ...
            unknown.append(key)
    missing = sorted(p for p, present in prefixes.items() if not present)
    ...
    return dict(bucket=bucket, scope_prefix='', observed_at=datetime.now(timezone.utc).isoformat(),
                ...
                examples=dict(unrecognized_keys=unknown[:MAX_EXAMPLES],
                              missing_metadata_prefixes=missing[:MAX_EXAMPLES], collision_names=collisions[:MAX_EXAMPLES]),
...
def inventory_coverage():
    ...
    try:
        ...
        for _ in range(MAX_PAGES):
            kwargs = dict(Bucket=cfg.bucket, Prefix='', MaxKeys=min(1000, MAX_OBJECTS - len(objects)))
            if token:
    ...
    report = build_coverage(objects, bucket=cfg.bucket, listing_complete=complete, stop_reason=reason)
    if reason == 'read_error':
        report['limitations'].append('Listing could not finish within its read budget. Check bucket-root LIST permission and Operations credentials; prefix-only permission may be insufficient.')
    return report
