...
Raw groups rows into **Processing queue**, **Needs action**, **Waiting for upload**, and **Completed / rejected**. Select the last group for rows displayed as **In Clean** or **Rejected**; completed rows preserve their earlier diagnostic under **Recorded status message**. Each group supports search, source/status filters and pagination. Playback re-lists all known delivery prefixes and omits verified processed copies and empty files. Raw displays pending-file counts and uploaded megabytes. Accepted hours and hourly estimates are in [Clean](../docs/OPS-CLEAN.md). Queue classification does not delete episodes or alter ownership, approvals, or payment history.

`GET /api/ops/state` returns `raw_backlog` for episodes with outstanding nonempty media in the latest bucket-scan snapshot. It excludes deleted and rejected episodes and sources fully covered by verified receipts; queued, running, blocked and incomplete uploads with pending media remain included. `known_seconds` sums positive, finite stored `Episode.duration_s` values once per episode, regardless of stereo files or repeated deliveries. This is an estimate from recorded metadata, not a fresh measurement of current media or a processing-time estimate. Existing nonzero episode durations are not replaced by a rescan.

`known_episodes` counts timed episodes; `unknown_episodes` counts those with invalid/missing durations or partial Clean coverage; `partial_episodes` is the subset with partial coverage; and `pending_episodes` is the total of known and unknown episodes. Partial episodes remain untimed because a full episode duration cannot measure just its outstanding segments. The API does not estimate their time from file counts or bytes. Missing or wrong-bucket inventory returns `raw_backlog: null`; a valid empty snapshot returns zero counts. The **Footage awaiting processing** card shows the known duration and separate unknown counts across all sources, unaffected by table filters or pagination. Scan again to refresh source inventory; the existing `last_scan` field identifies its age.

`raw_backlog.sources` partitions that backlog by confirmed business ID first, otherwise by contributor (`wearer_id`), with an **Unassigned** group when neither exists. Each entry carries `key`, `name`, `kind`, `country` and the same five duration/count fields as the overall summary. Keys use `business:<id>`, `wearer:<id>` or `wearer:null`, so matching display names do not merge identities. Business country comes from its preserved attribution; other groups have `country: null`. Missing contributor names display as `Contributor #<id>`. Only groups with pending episodes appear, ordered by descending known seconds, then name and key. Unknown and partially processed durations follow the same rules as the total; an empty backlog has `sources: []`.

The pipeline-token-authenticated `GET /api/ops/pipeline/health` separately returns `accumulated_data` for the 30-minute Slack reports: known recording time, episode counts and stored upload bytes by country, plus a grand total and unknown-measurement counts. It reads all non-deleted episode rows, including processed and held recordings, once per recording. Country comes from confirmed business provenance and preserved source prefixes. These totals measure accumulated uploads rather than the pending queue or retained Clean output; uploaded bytes are ledger volume rather than current bucket storage. See the [monitor runbook](../infra/raw_lifecycle/README.md#scheduled-health-monitor) for fields, attribution rules and deployment order.

Restoring a soft-deleted episode through `POST /api/ops/episodes/{recording}/restore` resets a rejected processing job only when its reason starts with `Previously removed by an operator: `. That job enters `uploading` with an awaiting-revalidation message and cleared lease; a fresh scan must diagnose its sources before a worker can claim it. Pending media in the existing inventory can immediately return to overall and source backlog counts. Other rejections and restore calls for episodes that are already active leave job state unchanged. Hard-deleted episodes remain unrestorable through this endpoint.
...
| `CONTRIBUTOR_PAYMENT_SHEET_URL` | for bank registration and enrolled-account deletion | Owner-executed Apps Script web-app `/exec` URL. See the [bank spreadsheet guide](../docs/contributors/bank-spreadsheet.md) for workbook and deployment setup. |
| `CONTRIBUTOR_PAYMENT_SHEET_SECRET` | for bank registration and enrolled-account deletion | Separate random secret of at least 32 characters, matching Apps Script's `BANK_DETAILS_SECRET`; authenticates timestamped bank requests. Never expose it to the browser or spreadsheet cells. |
