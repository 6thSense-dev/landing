...
The existing `FootageReview` attestation and `ActivityReview` task declaration are independent ledgers. Saving task spans does not mark footage reviewed for Payment or satisfy payout/recipient approval fences. Both legacy video-only runs and current multimodal runs retain their original manifests and contributor attribution.
