...
7. Korean contributors register payment details on the website after accepting the current spreadsheet-storage notice. The former mobile bank-write routes now return HTTP `410` and direct callers to the website. Full details go to the private **Bank details** tab; Ops shows a masked receipt. Existing Wise submissions retain their separate review/recovery workflow, and payment linking currently accepts KRW recipients only.
...
Korean individual contributors can use [Payment details](https://6thsense.dev/upload#payment-details) after signing in with a current contract. Camera approval is independent. They review the current summary and expandable provider/overseas-processing details, separately consent to collection/use and overseas transfer, then enter their account holder name, bank name and account number. Bank details never need to be emailed to staff. Payment setup remains optional for uploading.

`GET /api/contributor/bank/setup` returns only the caller's masked bank status and the versioned Korean/English notice. `POST /bank/web/requirements` and `POST /bank/web` require its exact version/hash, language and both requested choices (`collects_details` and `international_transfer`). The authenticated backend writes full bank details and consent to the private workbook, then stores a masked `sheet_saved` receipt. It collects no birth date or address and makes no Wise request. Responses prohibit caching. See the [bank spreadsheet guide](contributors/bank-spreadsheet.md) for configuration, notice publication, retries, corrections and deletion.

Finance reviews new requests at **Ops → Users → Mobile contributor requests** and follows **Open contributor spreadsheet → Bank details** to inspect the saved account. A lost response can be recovered with **Check saved status** or a safe retry; one row per contributor prevents duplicate saves. Existing manually linked recipients still show as verified, and historical Wise submissions still require operator reconciliation and separate **Verify and link recipient** approval. Registration does not approve footage, calculate earnings or send money.

Historical verification: the former Wise requirements route was checked read-only on 2026-09-18. Its automated tests used simulated recipients and did not create real recipients or send payments. That check does not verify the replacement spreadsheet bridge. See [Wise's KRW guide](https://wise.com/help/articles/2932331/guide-to-krw-transfers) for verification that may separately apply to an actual Wise payment.

The former website-payment release passed 64 focused backend tests and 60 browser checks across mobile, tablet and desktop. That historical coverage included consent/version enforcement, account isolation, masked persistence, ambiguous writes, recovery after operator-confirmed non-creation, pre-camera setup, unchanged uploads and existing Ops recipient approval. Its Korean notice and form were visually checked at 375 and 1280 pixels. Replacement-flow validation is described in the bank spreadsheet guide.
...
| `GET /api/contributor/bank/setup` | Masked bank status and current bilingual payment notice; recovers a spreadsheet receipt if its earlier response was lost. |
| `POST /api/contributor/bank/web/requirements` / `POST /api/contributor/bank/web` | Three bank fields and spreadsheet submission with current notice consent; saving also requires a unique `operation_id`. |
| `POST /api/contributor/bank/requirements` / `POST /api/contributor/bank` | Retired mobile writes: HTTP `410`, `use_website_bank_registration`. No Wise calls. |
...
Historical Wise recipient submissions were committed before their provider request. Existing unknown outcomes remain held, including after a process crash. Never release a hold merely because the app timed out. In Ops, resolve it only after confirming a specific recipient or confirming that no recipient was created. Resolution records operator, timestamp, provider profile/environment and a note without bank details. A confirmed no-create outcome permits a new website spreadsheet submission with a new operation ID. Payout linking/approval remain separate.
