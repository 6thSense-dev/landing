...
- **Sieve** shows retained Clean hours, verified inherited hours, country/entity/operator-task diversity and per-recording copy status through September 25, 2026, Los Angeles time. Open a verified episode to inspect video, IMU, metadata, calibration and pipeline action labels together; pipeline labels are separate from optional operator task assignments. Its independent worker copies Clean MP4, validated metadata with its provenance and calibration, then waits five minutes before the next pass. IMU and task reports remain in Clean. Customer delivery and acceptance remain separate. See [Sieve collection and inheritance](SIEVE-CLEAN-INHERITANCE.md).
