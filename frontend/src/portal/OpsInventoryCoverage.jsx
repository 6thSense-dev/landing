...
export default function OpsInventoryCoverage() {
  ...
  const [sourceId, setSourceId] = useState('operations');
  const [registryBusy, setRegistryBusy] = useState(false);
  const controller = useRef(null);
  const registryController = useRef(null);
  useEffect(() => () => { controller.current?.abort(); registryController.current?.abort(); }, []);
  const selected = sources?.find(source => source.id === sourceId);

  async function loadSources() {
    registryController.current?.abort();
    controller.current?.abort();
    setReport(null); setError(''); setBusy(false); setRegistryBusy(true); setSources(null);
    const request = new AbortController();
    registryController.current = request;
    const response = await portalFetch('/api/ops/inventory-sources', { signal: request.signal });
    if (request.signal.aborted) return;
    setRegistryBusy(false);
    if (!response.ok || !Array.isArray(response.data?.sources) || !response.data.sources.some(s => s.id === 'operations')) {
      setError('Recording sources could not be loaded. Coverage is unknown. Retry the source list.');
      return;
    }
    setSources(response.data.sources);
    setSourceId('operations');
  }

  function changeSource(id) {
    controller.current?.abort();
    setSourceId(id); setReport(null); setError(''); setBusy(false);
  }
  ...
  async function check() {
    ...
    setError("");
    setBusy(true);
    const response = await portalFetch(`/api/ops/inventory-coverage?source_id=${encodeURIComponent(sourceId)}`, { signal: request.signal });
    if (request.signal.aborted) return;
    setBusy(false);
    if (!response.ok || typeof response.data?.listing_complete !== "boolean" || response.data.source_id !== sourceId) {
  ...
  return (
    <details className="ops-inventory" onToggle={event => { if (event.target === event.currentTarget && event.currentTarget.open && sources === null && !registryBusy) loadSources(); }}>
      ...
      <div className="ops-inventory-body">
        ...
        {sources && <label>Recording source <select aria-label="Recording source" value={sourceId} onChange={e => changeSource(e.target.value)}>
          {sources.map(source => <option key={source.id} value={source.id}>{source.label} · {source.expected_owner || 'account not specified'}</option>)}
        </select></label>}
        {selected && <p className="ops-hint"><code>s3://{selected.bucket}/{selected.prefix}</code> · Expected account: {selected.expected_owner || 'not configured'}. Access has not been established by listing this source.</p>}
        <button type="button" onClick={loadSources} disabled={registryBusy}>{registryBusy ? 'Loading sources…' : sources ? 'Refresh source list' : 'Retry source list'}</button>
        <button type="button" onClick={check} disabled={busy || registryBusy || !selected}>
        ...
        {report && <section aria-label="Inventory coverage result" aria-live="polite">
          <p className={report.listing_complete ? "ops-inventory-status" : "ops-inventory-warning"}>
            {report.listing_complete
              ? "Listing completed for the selected source prefix."
          ...
          <dl className="ops-inventory-counts">
            ...
            {report.recordings_metadata_outside_scope > 0 && <div><dt>Metadata outside checked prefix · unknown</dt><dd>{count(report.recordings_metadata_outside_scope)}</dd></div>}
          ...
          {[
            ...
            ["Folders whose metadata is outside the checked prefix", report.examples?.outside_scope_prefixes],
