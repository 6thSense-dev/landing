export function cleanTotals(runs) {
  return runs.reduce((total, run) => ({
    ...
    due: total.due + (!run.paid ? run.estimated_krw ?? 0 : 0),
    missing: total.missing || (!run.paid && run.estimated_krw == null),
...
export function groupCleanFootage(runs, collections) {
  ...
  const ensure = (key, wearerId) => {
    if (!groups.has(key)) groups.set(key, { key, wearer_id: wearerId, runs: [], collections: [] });
    return groups.get(key);
  };
  for (const run of runs) {
    const key = run.wearer_id == null ? `unassigned:${run.device_id}` : `wearer:${run.wearer_id}`;
    ensure(key, run.wearer_id).runs.push(run);
  }
