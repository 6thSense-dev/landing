...
`s3://6thsense-catalog-media/v1/`. Bundle-relative paths beginning with `media/` are
...
1. Authenticate as a catalog user and `GET /api/catalog`; confirm the manifest is
   returned successfully and preview URLs point at `6thsense-catalog-media`.
...
Redeploy the preceding application revision. The catalog tier remains at
`6thsense-catalog-media/v1/`; the package variables may remain recorded for a forward
switch because the preceding revision ignores them.
