...
## 2026-09-16 — Close generic-import metadata race

- Reserved the handed-off `raw-clean-20260915-*` runs from the generic Clean scanner so the authenticated pipeline bridge owns their first import.
- Lifecycle reconciliation now binds an adopted or current successful Clean job to its pinned plan, committed manifest, conversion receipt, portal inventory, and the exact nonempty Raw media identities and byte counts before supplementing original metadata.
- A pinned processed metadata/provenance proof is saved before retirement. Partial retirement retries validate that durable Clean proof without rereading Raw versions that may already be deleted, and an API-import-before-state-save crash reconstructs the proof from the owned job.
- Metadata copy or provenance errors stop retirement. Already-retired states remain untouched, zero-byte archived placeholders remain outside conversion evidence, and historical imports not owned by the lifecycle coordinator retain their existing reconciliation behavior as an explicit compatibility boundary.
- Verification: `tests/raw_lifecycle` — 71 passed; `backend/tests/test_ops_clean.py` — 28 passed; independent adversarial review approved the lifecycle proof and crash-resume design. No AWS or deployment mutations were performed.
